...
With a bit of monkey-patching (a few pointers on the Cloudflare Pages website during setup, a few `.nojekyll` files for Github Pages and a symlink from the `/docs` dir to Cloudflare's `/dist` dir, and a few CLI commands for Google Firebase), was able to get this site running across 3 hosts with ease.
